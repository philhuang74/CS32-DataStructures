...
void removeBad(vector<Movie*>& v)
{
    ...
    while(it != v.end()){
        if ((*it)->rating() < 50){
            delete *it; //first delete the dynamically allocated object iterator points to
            it = v.erase(it); //erase returns the next iterator even though it makes iterator invalid when called
        }
        else{
