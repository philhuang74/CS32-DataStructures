...
Map::Map(int max){
    if (max < 0){
//        cerr << "Please give number a non-negative number";
        exit(1);
    }
    m_size = 0;
    m_max = max;
    m_keyValuePairs = new KeyValuePair[max];
}
...
Map::Map(const Map& other){
    m_size = other.m_size;
    m_max = other.m_max;
    m_keyValuePairs = new KeyValuePair[m_max];

    for(int i = 0; i < m_size; i++){
        m_keyValuePairs[i] = other.m_keyValuePairs[i];
    }
} //copy constructor
...
Map& Map::operator=(const Map& other){
    ...
    else{ //other is not the same as this
        Map otherMap(other); //otherMap = other
        this->swap(otherMap);
    }
    return *this;
} //assignment operator
...
bool Map::insert(const KeyType& key, const ValueType& value){ //###Should we use setKeyValue to be pass by copy?
    if (m_size >= m_max){ //changed from DEFAULT_MAX_ITEMS to m_max
        return false;
    }
    for (int i = 0; i < m_size; i++){
...
void Map::swap(Map& other){
    ...
    int temp_max = this->m_max;
    this->m_max = other.m_max;
    other.m_max = temp_max;

    KeyValuePair* temp_ptr = this->m_keyValuePairs;
    this->m_keyValuePairs = other.m_keyValuePairs;
    other.m_keyValuePairs = temp_ptr;
...
void Map::dump() const{
    cerr << "\n";
    for(int i = 0; i < m_size; i++){
        cerr << i << " Key: " << m_keyValuePairs[i].getKey() << " " << "Value: " << m_keyValuePairs[i].getValue();
        cerr << "\n";
    }
    cerr << "------------------------------";
    cerr << "\n";
